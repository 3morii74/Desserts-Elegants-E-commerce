...
<body>
    <div class="container p-8 mx-auto mt-12">
        <div class="w-full overflow-x-auto">
            <div class="my-2">
                <h3 class="text-xl text-teal-800 font-bold tracking-wider">Shopping Cart | {{ count($items) }} Items</h3>
            </div>
                    ...
                    <tr data-price="{{ $item['price'] }}">
                        <td>
                            <div class="flex justify-center">
                                <img src="{{ Storage::url($item['image']) }}" class="object-cover h-32 w-32 rounded-2xl p-2" alt="image" />
                            </div>
                        </td>
                        ...
                        <td class="p-4 px-6 text-center whitespace-nowrap text-teal-800">
                            <div>
                                ...
                                <input id="qty_{{ $item['id'] }}" type="text" name="qty" value="{{ $item['quantity'] }}" class="w-12 text-center text-teal-800 bg-gray-100 outline-none" onchange="updateSubtotal({{ $item['id'] }})" />
                                <button onclick="incrementQuantity({{ $item['id'] }})">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="inline-flex w-6 h-6 text-teal-800" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </button>
                            </div>
                        </td>
                        <td class="p-4 px-6 text-center whitespace-nowrap text-teal-800" id="subtotal_{{ $item['id'] }}">{{ $item['price'] * $item['quantity'] }} L.E</td>
                        <td class="p-4 px-6 text-center text-teal-800 whitespace-nowrap">
            ...
            <div class="mt-4">
                <div class="py-4 rounded-md shadow">
                    <div class="flex items-center justify-between px-4 py-2 mt-3 border-t-2">
                        ...
                        <span id="total2" class="text-2xl text-teal-800 font-bold">{{ $subtotal }} L.E</span>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-4">
                <form method="POST" action="{{ route('item.checkOut') }}">
                    ...
                    <button type="submit" class="w-full py-2 text-center text-purple-50 bg-teal-800 rounded-md shadow hover:bg-teal-600">
                        Proceed to Checkout
                    </button>
                </form>
            </div>
        </div>
        <a href="{{ route('Home') }}"><button class="w-20 py-2 text-center text-purple-50 bg-teal-800 rounded-md shadow hover:bg-teal-600">
             Go Back
        </button></a>
    </div>
</body>
